...
CREATE OR REPLACE VIEW `editor_rows` AS (
	SELECT resource.resource_id as 'id', title AS 'name', categories, services, street_address, zipcode, phone, email, website, description, documents, requirements, insurance, opHours, contacts AS 'contactList', admin_id, username AS 'lastUpdate_admin', updates_w_admin.date as 'lastUpdate' FROM resource
	JOIN category_grouped_editor ON resource.resource_id=category_grouped_editor.resource_id
	JOIN service_grouped_editor ON resource.resource_id=service_grouped_editor.resource_id
...
CREATE OR REPLACE VIEW `search_rows` AS (
	SELECT resource.resource_id AS 'id', title AS 'name', categories, services, street_address, zipcode, phone, email, website, description, documents, requirements, insurance, opHours, contacts AS 'contactList', updates.date AS 'lastUpdate' FROM resource
